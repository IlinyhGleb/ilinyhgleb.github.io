...
5. Задайте переменную `x=5`. Введите выражение `x/7+5**3` и нажмите `Enter`.
   ...
   {{<details summary="Результат выражения">}} 
   ```Python
   >>>x=5
   >>>x/7+5**3
   125.71428571428571
...
8. Посчитайте выражение \( \cos{y}+3\sin{x} \). Для ввода функции \( \cos{y} \) используется код `math.cos(y)`, аналогично для \( \sin{x} \).
   ...
   >>>x=5
   >>>y=7
   >>>math.cos(y)+3*math.sin(x)
   -2.1228705696461105
   ```
   {{</details>}}

9. Для выхода из Python используйте комбинацию клавиш `Ctrl+z` и нажмите `Enter`.
...
2. Создайте новый проект:
   ...
   При этом автоматически будет создано новое [виртуальное окружение]({{< ref "#venv" >}}) в папке `venv`.
...
4. Запишите в новом файле код вычисления значения функции:
   ...
   используя пользовательский ввод и вывод для переменных \(x\), \(y\), \(a\), \(b\), как в [предыдущем разделе]({{< ref "#python_idle" >}}).
