...
{{< tikz >}}
	...
	\begin{tikzpicture}
		...
		\node[align=left] at (2.0,3.0) (label) {$\begin{cases} 
		...
		\node[align=left] at (7.5,2.75) (label) {\scriptsize{=ЕСЛИ(\colorbox{gray!20}{$x<=0$};\colorbox{green!20}{$0$};ЕСЛИ(\colorbox{gray!40}{$x>4$};\colorbox{yellow!20}{$\cos(x-4)$};\colorbox{red!20}{$x/4$}))}};
	\end{tikzpicture}
...
$$ z(x) = \begin{cases} -x-1, \quad x \le -1 \\ x-1, \quad x \ge 1 \\ -x^2+1, \quad x > 0, x < 1 \end{cases} $$
...
Реализуйте таблицу истинности в электронных таблицах.
